...
In `Stencil`, [DataContractSerializer](https://learn.microsoft.com/en-us/dotnet/api/system.runtime.serialization.datacontractserializer?view=net-8.0) serializes symbols and their properties into a XAML file. The following Stencil properties are included in the serialization process, ensuring that the Stencil settings can be restored regardless of the Stencil's current visibility:
...
By default, when deserializing a Stencil, symbols and their properties are loaded from the saved stream. To restore the previously saved Stencil settings, call `stencil.Load(stream, restoreStencilSettings: true)`. This ensures that the Stencil settings are deserialized and applied, regardless of the Stencil's current visibility.

N> Before calling `Load`, the Stencil may already contain `SymbolGroup`s. The loaded stream is merged into the existing groups — symbols with the same `Key` are replaced and new groups are appended.
...
You do not need to explicitly specify `false` for the second parameter of the `Load` method, as it is the default value.
...
## Exporting and Importing Symbol Groups in the Stencil

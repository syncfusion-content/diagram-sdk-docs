...
N> This guide adds the Vue Diagram component directly in `src/App.vue` for simplicity. In a real application, you can move the Diagram code to a page component such as `src/pages/IndexPage.vue` (the default page generated by Quasar when the starter template includes routing).
...
N> The Vue Diagram component must have a valid height. If the height is not set, the Diagram canvas may not be visible.

...
Node annotations support the following positioning properties through the [`ShapeAnnotation`](https://ej2.syncfusion.com/react/documentation/api/diagram/shapeAnnotationModel) class:
...
The [`offset`](https://ej2.syncfusion.com/react/documentation/api/diagram/shapeannotationmodel#offset) property positions annotations using fractional values between 0 and 1. The offset represents the relative position within the node boundaries, where (0,0) is the top-left corner and (1,1) is the bottom-right corner. The default offset is (0.5, 0.5), which centers the annotation.
...
Annotation offset values can be modified dynamically during application execution. To update the offset, access the target annotation via `nodes[0].annotations[0].offset`, modify its `x` and `y` values, then call the `dataBind()` method to apply changes immediately.
...
After determining the annotation position using offset values, the [`horizontalAlignment`](https://ej2.syncfusion.com/react/documentation/api/diagram/annotationmodel#horizontalalignment) property sets how the annotation is horizontally aligned at the annotation position determined from the fraction values. The [`verticalAlignment`](https://ej2.syncfusion.com/react/documentation/api/diagram/annotationmodel#verticalalignment) property controls how the annotation aligns at that calculated position.

...
N> Many of the interaction events expose a `state` field on their event arguments. Common values include `Changing` (cancelable, fires before the operation completes — e.g. selection, collection change), `Progress` (fires during the operation — e.g. drag, resize, rotate), and `Completed` (fires once the operation finishes).
...
The following code example demonstrates how to handle the [`click`](https://ej2.syncfusion.com/react/documentation/api/diagram#click) event in the diagram. This is the diagram's `click` event, not the native DOM `click` event. For a double-click action, refer to the related [`doubleClick`](https://ej2.syncfusion.com/react/documentation/api/diagram#doubleclick) event.
...
Triggered when a node's selection state changes (selected or deselected), firing during both the selection process and completion to provide control over selection behavior.
...
Triggered when any property of a node is modified programmatically or through user interaction. This event is useful for tracking changes and implementing custom validation logic. The event arguments include `propertyName`, `oldValue`, and `newValue` to identify what changed.
...
Triggered when nodes are added to or removed from the diagram dynamically. This event provides control over diagram modifications and enables validation before collection changes occur. The event arguments include a `cause` field that indicates the operation type: `add`, `remove`, or `clear`.
...
```
    ...
    if (args.state === 'Changing') {
      //Prevents collection change — adding or deleting nodes
      args.cancel = true;
    }
  },
...
The [`mouseEnter`](https://ej2.syncfusion.com/react/documentation/api/diagram#mouseenter) event is triggered once when the mouse cursor first enters a node's boundary area.

### Mouse Over Event

The [`mouseOver`](https://ej2.syncfusion.com/react/documentation/api/diagram#mouseover) event is triggered when the mouse cursor moves over a node's surface area.
